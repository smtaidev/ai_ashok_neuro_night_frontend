...
const AddMeetingModal: React.FC<AddMeetingModalProps> = ({
  ...
  const [meetingTitle, setMeetingTitle] = useState("");
  const [location, setLocation] = useState("");
  const [type, setType] = useState<"Annual" | "Board" | "Monthly" | "Quarterly" | "">("");
  const [date, setDate] = useState(today);
  const [startTime, setStartTime] = useState("06:00");
  const [endTime, setEndTime] = useState("07:00");
  const [meetingLength, setMeetingLength] = useState("");
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    // Ensure type is properly typed before using it
    if (!type || !["Annual", "Board", "Monthly", "Quarterly"].includes(type)) {
      alert("Please select a valid meeting type");
      return;
    }
    ...
    const meetingData = {
      ...
      companyName,
      location,
      type: type as "Annual" | "Board" | "Monthly" | "Quarterly",
      meetingDate: startDateTime.toISOString(),
      startDate: startDateTime.toISOString(),
      endDate: endDateTime.toISOString(),
      meetingLength,
        ...
        <form onSubmit={handleSubmit} className="p-6 grid grid-cols-1 gap-y-4">
          ...
          <div>
            ...
            <select
              className="mt-1 block w-full border border-gray-300 rounded-md shadow-sm py-2 px-3 focus:outline-none focus:ring-blue-500 focus:border-blue-500 sm:text-sm"
              value={type}
              onChange={(e) => setType(e.target.value as "Annual" | "Board" | "Monthly" | "Quarterly" | "")}
              required
            >
              <option value="">Select type...</option>
              <option value="Monthly">Monthly</option>
