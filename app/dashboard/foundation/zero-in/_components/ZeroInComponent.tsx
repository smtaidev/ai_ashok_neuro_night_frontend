...
export default function ZeroInComponent() {
  ...
  console.log("zeroInRes", zeroInRes);
  const [sections, setSections] = useState<Section[]>([...ZeroInSectionsData]);
  const [patchZeroIn, { isLoading }] = usePatchFoundationZeroInMutation();
  ...
  const handleEditClick = async (section: Section) => {
    ...
    try {
      ...
      if (response.data?.data && response.data.data.length > 0) {
        ...
        if (fieldName === 'targetCustomer') latestContent = latestZeroInData.targetCustomer || '';
        if (fieldName === 'keyCustomerNeed') latestContent = latestZeroInData.keyCustomerNeed || '';
        if (fieldName === 'valueProposition') latestContent = latestZeroInData.valueProposition || '';
        ...
        setSections(prev =>
          prev.map(sec => sec.id === section.id ? { ...sec, content: latestContent } : sec)
        );
      } else {
        setActiveSection(section);
        setEditedContent(section.content);
  ...
  const handleSave = async () => {
    ...
    const wordCount = editedContent.trim().split(/\s+/).filter(Boolean).length;
    if (wordCount > 1000) {
      toast.error(`${activeSection.title} exceeds 1000 words. Please revise.`);
      return;
    }

    const payload = { [fieldName]: editedContent };

    try {
      await patchZeroIn(payload).unwrap();
      setSections(prev =>
        prev.map(sec => sec.id === activeSection.id ? { ...sec, content: editedContent } : sec)
      );
      setOpen(false);
      toast.success(`${activeSection.title} updated successfully`);
      localStorage.removeItem("zeroInData");
  ...
  const handleMoreInfoClick = () => {
    if (activeSection) setOpenDrawerId(activeSection.id);
  };
  const handleCloseDrawer = () => setOpenDrawerId(null);
