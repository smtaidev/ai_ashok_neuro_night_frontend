...
import { Card, CardContent } from "@/components/ui/card";
import { Button } from "@/components/ui/button";
import { Eye, Edit, Trash, Plus, X, Loader2 } from "lucide-react";
import { useGetAllOrganizationUsersQuery } from "@/redux/api/OrganizationUser/organizationUserApi";
import { Dialog, DialogContent, DialogHeader, DialogTitle } from "@/components/ui/dialog";
import { Input } from "@/components/ui/input";
import { Label } from "@/components/ui/label";
...
const TeamMemberPage = ({ teamName }: { teamName: string }) => {
  ...
  const [viewModal, setViewModal] = useState<ITeamMember | null>(null);
  const [editingTeamMember, setEditingTeamMember] = useState<ITeamMember | null>(null);
  const [deleteConfirmMember, setDeleteConfirmMember] = useState<ITeamMember | null>(null);
  ...
  const handleDelete = async (memberId: string) => {
    ...
    try {
      ...
      toast.success("Member deleted successfully");
      refetch();
      setDeleteConfirmMember(null); // close modal
    } catch {
      toast.error("Failed to delete member");
    }
  };
    ...
    <div className="p-6">
                  ...
                  <tr key={member._id} className="border-b border-gray-800">
                    ...
                    <td className="px-6 py-4 text-right flex justify-end gap-2">
                      ...
                      <Button
                        size="sm"
                        className="bg-red-600 hover:bg-red-700 text-white"
                        onClick={() => setDeleteConfirmMember(member)} // ✅ open confirm modal
                        disabled={isDeleting}
                      >
                        <Trash className="w-4 h-4" />
                      </Button>
      ...
      <Dialog open={!!deleteConfirmMember} onOpenChange={() => setDeleteConfirmMember(null)}>
        <DialogContent className="max-w-md">
          ...
          <p className="text-lg text-[#231F20] mt-2">
            Are you sure you want to delete{" "}
            <strong>{deleteConfirmMember?.userName}</strong> (
            {deleteConfirmMember?.email})? This action cannot be undone.
          </p>
          <div className="flex justify-end gap-2 mt-4">
            <Button
              variant="outline"
            ...
            <Button
              variant="destructive"
              onClick={() => deleteConfirmMember && handleDelete(deleteConfirmMember._id)}
              disabled={isDeleting}
            >
              {isDeleting ? (
                <>
