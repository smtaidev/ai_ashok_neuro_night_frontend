...
import {
  ...
  useGetAiSwotQuery,
  useGetAiTrendQuery,
} from "@/redux/api/clarhetai-recomandation/clarhetaiApi";
...
const ClarhetAiRecommendations: React.FC<RecommendationsProps> = ({
  ...
  const renderTrendContent = () => {
    ...
    const isValidContent = (content: string | string[]) => {
      if (Array.isArray(content)) {
        return (
          content.length > 0 && !content.some((item) => item.includes("Error:"))
        );
      }
    ...
    return (
      <div className="space-y-4 max-h-96 overflow-y-auto">
        <div className="mb-4">
          ...
          >
            Company:{" "}
            <span className="font-medium">{trendData.data.companyName}</span>
          </p>
        </div>
          ...
          <div className="bg-indigo-50 p-3 rounded-lg">
            ...
            <div className="space-y-2">
              {radar_executive_summary.map((summary, index) => (
                <p
                  key={index}
                  className="text-base text-indigo-700 flex items-start"
                >
        ...
        {radar_recommendation && radar_recommendation.length > 0 && (
          <div className="bg-violet-50 p-3 rounded-lg">
            <h4 className="text-base font-semibold text-violet-800 mb-2">
            ...
            <div className="space-y-2">
              {radar_recommendation.map((recommendation, index) => (
                <p
                  key={index}
                  className="text-base text-violet-700 flex items-start"
                >
        ...
        {isValidContent(summary.significant_risks) && (
          <div className="bg-amber-50 border-l-4 border-amber-500 p-3 rounded-r-lg">
            <h4 className="text-base font-semibold text-amber-800 mb-1">
              Significant Risks
            </h4>
            <p className="text-base text-amber-700">
              {summary.significant_risks}
            </p>
          </div>
        )}
        ...
        {isValidContent(trend_synthesis) && (
          <div className="bg-cyan-50 p-3 rounded-lg">
            <h4 className="text-base font-semibold text-cyan-800 mb-2">
              Trend Analysis
            </h4>
            <div className="space-y-1">
              {trend_synthesis
                .filter((trend) => !trend.includes("Error:"))
                .map((trend, index) => (
                  <p
                    key={index}
                    className="text-base text-cyan-700 flex items-start"
                  >
        ...
        {isValidContent(strategic_opportunities) && (
          <div className="bg-teal-50 p-3 rounded-lg">
            ...
            <div className="space-y-1">
              {strategic_opportunities
                .filter((opp) => !opp.includes("Error:"))
                .map((opportunity, index) => (
                  <p
                    key={index}
                    className="text-base text-teal-700 flex items-start"
                  >
        ...
        {isValidContent(early_warnings) && (
          <div className="bg-red-50 p-3 rounded-lg">
            ...
            <p className="text-base text-red-700">{early_warnings}</p>
          </div>
        )}
        ...
        {isValidContent(analyst_recommendations) && (
          <div className="bg-purple-50 p-3 rounded-lg">
            ...
            <p className="text-base text-purple-700">{analyst_recommendations}</p>
          </div>
        )}
          ...
          !isValidContent(analyst_recommendations) && (
            <div className="bg-gray-50 p-4 rounded-lg text-center">
              ...
              <p className="text-base text-gray-500">
                Some analysis sections may need more relevant business data
              </p>
            </div>
          )}
      </div>
  ...
  const renderChallengeContent = () => {
      ...
      return (
        <div className="h-48 flex items-center justify-center">
          ...
          <span className="ml-2 text-gray-500">
            Loading challenge recommendations...
          </span>
        </div>
      );
    }
    ...
    const recommendations = parseRecommendations(
      challengeData.data.recommendations
    );
        ...
        <div className="mb-4">
          <p className="text-base font-semibold text-gray-700 mb-3">
            ...
            <span className="text-base font-semibold text-gray-700">
              {challengeData.data.companyName}
            </span>
          </p>
        </div>
            ...
            <div className="bg-green-50 p-3 rounded-lg">
              <div className="flex justify-between items-center mb-1">
                ...
                <span className="text-lg font-bold text-green-600">
                  {scores.strengths.value}/10
                </span>
              </div>
              <p className="text-base text-green-700">
                {scores.strengths.rationale}
              </p>
            </div>
            ...
            <div className="bg-red-50 p-3 rounded-lg">
              <div className="flex justify-between items-center mb-1">
                ...
                <span className="text-lg font-bold text-red-600">
                  {scores.weaknesses.value}/10
                </span>
              </div>
              <p className="text-base text-red-700">
                {scores.weaknesses.rationale}
              </p>
            </div>
            ...
            <div className="bg-blue-50 p-3 rounded-lg">
              <div className="flex justify-between items-center mb-1">
                ...
                <span className="text-lg font-bold text-blue-600">
                  {scores.opportunities.value}/10
                </span>
              </div>
              <p className="text-base text-blue-700">
                {scores.opportunities.rationale}
              </p>
            </div>
            ...
            <div className="bg-yellow-50 p-3 rounded-lg">
              <div className="flex justify-between items-center mb-1">
                ...
                <span className="text-lg font-bold text-yellow-600">
                  {scores.threats.value}/10
                </span>
              </div>
              <p className="text-base text-yellow-700">
                {scores.threats.rationale}
              </p>
            </div>
          </div>
        </div>
            ...
            <div>
              ...
              <ul className="space-y-1">
                {recommendations.strengths.map((rec, index) => (
                  <li
                    key={index}
                    className="text-base text-gray-600 flex items-start"
                  >
  ...
  return (
    <div className="my-8 rounded-lg  ml-6 p-6 bg-white">
      {/* Header Section */}
      <div className="mb-8">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="bg-white border border-gray-200 rounded-lg p-6 min-h-[300px]">
          <h2 className="text-xl font-bold text-[#0b1c33] mb-4">SWOT</h2>
          <div className="text-base text-gray-700">{renderSwotContent()}</div>
        </div>

        {/* Trends Section - Now with API data */}
        <div className="bg-white border border-gray-200 rounded-lg p-6 min-h-[300px]">
          <h2 className="text-xl font-bold text-[#0b1c33] mb-4">Trends</h2>
          <div className="text-base text-gray-700">{renderTrendContent()}</div>
        </div>
